...
VALUES ('client.sendMessage(''Здравствуйте! Это автоответ. Вам напишет первый освободившийся оператор.'')', 'ENABLED',
       ...
       ('task.create(message.text)', 'ENABLED',
        ...
        'client.openTasks.size() = 0 and client.incomeMessages.size() > 1 and !client.incomeMessages.last().text.lower().containsAny(''спасибо'', ''благодарю'', ''спс'')',
        'Авто-новая заявка', 1, 'MESSAGE_INCOMING'),
       ...
       ('client.sendMessage(''Здравствуйте! Мы получили ваше сообщение. Сейчас нерабочее время, оператор ответит вам в ближайшее рабочее время.'')',
        ...
        'hour(now()) < 6 and hour(now()) >= 15', 'Сообщение вне рабочего времени', 3, 'MESSAGE_INCOMING'),
...
INSERT INTO task_filter (label, filter_join_operator, selected_options)
VALUES ('Требуют внимания', 'OR', '[
  {
    "label": "Тег",
    "selectedOptions": [
      "SLA скоро нарушится",
